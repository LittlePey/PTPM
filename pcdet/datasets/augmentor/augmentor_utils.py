...
def random_local_translation_along_x(gt_boxes, points, offset_range, return_offset=False, noise_offsets=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    for idx, box in enumerate(gt_boxes):
        if noise_offsets is None:
            ...
            offset = noise_offsets[idx]
        # augs[f'object_{idx}'] = offset
        points_in_box, mask = get_points_in_box(points, box)
        points[mask, 0] += offset
        
        gt_boxes[idx, 0] += offset
        noise_local_offset.append(offset)
        # if gt_boxes.shape[1] > 7:
        #     gt_boxes[idx, 7] += offset

    noise_local_offset = np.array(noise_local_offset, dtype=points.dtype)
    if return_offset:
        return gt_boxes, points, noise_local_offset
    else:
        return gt_boxes, points


def random_local_translation_along_y(gt_boxes, points, offset_range, return_offset=False, noise_offsets=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
...
def random_local_translation_along_z(gt_boxes, points, offset_range, return_offset=False, noise_offsets=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
...
def local_scaling(gt_boxes, points, scale_range, return_scale=False, noise_scales=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C),
    ...
    for idx, box in enumerate(gt_boxes):
        ...
        noise_local_scale.append(noise_scale)

    noise_local_scale = np.array(noise_local_scale, dtype=points.dtype)
    if return_scale:
        return gt_boxes, points, noise_local_scale
    else:
        return gt_boxes, points


def local_rotation(gt_boxes, points, rot_range, return_rot=False, noise_rots=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    noise_local_rot = []
    for idx, box in enumerate(gt_boxes):
        if noise_rots is None:
            noise_rotation = np.random.uniform(rot_range[0], rot_range[1])
        else:
            noise_rotation = noise_rots[idx]
        ...
        noise_local_rot.append(noise_rotation)

    noise_local_rot = np.array(noise_local_rot, dtype=points.dtype)
    if return_rot:
        return gt_boxes, points, noise_local_rot
    else:
        return gt_boxes, points
