...
from . import augmentor_utils, database_sampler, database_pseudo_sampler
...
class DataAugmentor(object):
    ...
    def pseudo_sampling(self, config=None):
        post_fix = config.get('POST_FIX', '')
        db_sampler = database_pseudo_sampler.DataBaseSampler(
            root_path=Path(str(self.root_path).replace('lpart', 'upart')) if post_fix == '' else Path(str(self.root_path).replace('lpart', 'upart')+'_'+post_fix),
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
        )
    ...
    def random_local_translation(self, data_dict=None, config=None):
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            ...
            gt_boxes, points, noise_local_offset = getattr(augmentor_utils, 'random_local_translation_along_%s' % cur_axis)(
                gt_boxes, points, offset_range, return_offset=True
            )
            data_dict['noise_local_offset_%s' % cur_axis] = noise_local_offset
    ...
    def random_local_rotation(self, data_dict=None, config=None):
        ...
        if not isinstance(rot_range, list):
            rot_range = [-rot_range, rot_range]
        gt_boxes, points, noise_local_rot = augmentor_utils.local_rotation(
            data_dict['gt_boxes'], data_dict['points'], rot_range=rot_range, return_rot=True
        )

        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        data_dict['noise_local_rot'] = noise_local_rot
        return data_dict
    ...
    def random_local_scaling(self, data_dict=None, config=None):
        ...
        gt_boxes, points, noise_local_scale = augmentor_utils.local_scaling(
            data_dict['gt_boxes'], data_dict['points'], config['LOCAL_SCALE_RANGE'], return_scale=True
        )

        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        data_dict['noise_local_scale'] = noise_local_scale
        return data_dict
    ...
    def random_local_pyramid_dropout(self, data_dict=None, config=None):
        ...
        gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
        gt_boxes, points, _ = augmentor_utils.local_pyramid_dropout(gt_boxes, points, config['DROP_PROB'])
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        return data_dict

    def random_local_pyramid_sparsify(self, data_dict=None, config=None):
        """
        Refer to the paper:
            SE-SSD: Self-Ensembling Single-Stage Object Detector From Point Cloud
        """
        ...
        gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
        gt_boxes, points, _ = augmentor_utils.local_pyramid_sparsify(gt_boxes, points,
    ...
    def forward(self, data_dict, unlabel_frame=False):
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
        ...
        for cur_augmentor in self.data_augmentor_queue:
            sampler_cfg = getattr(cur_augmentor, 'sampler_cfg', None)
            if sampler_cfg is None:
                data_dict = cur_augmentor(data_dict=data_dict)
            else:
                label_unlabel = sampler_cfg.get('LABEL_UNLABEL', [True, False])
                if unlabel_frame is False:
                    data_dict = cur_augmentor(data_dict=data_dict, use_db_sample=label_unlabel[0])
                else:
                    data_dict = cur_augmentor(data_dict=data_dict, use_db_sample=label_unlabel[1])
