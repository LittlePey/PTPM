...
class Octopus(object):
    ...
    def __init__(self, dataset_root, dataset_ceph=None, petrel_client=None):
        self.dataset_root = dataset_root
        self.data_root = osp.join(self.dataset_root, 'data')

        self.dataset_ceph = dataset_ceph
        self.data_ceph = osp.join(self.dataset_ceph, 'data')
        self.petrel_client = petrel_client
    ...
    def load_point_cloud(self, seq_id, frame_id):
        if self.data_ceph is None:
            bin_path = osp.join(self.data_root, seq_id, 'lidar_roof', '{}.bin'.format(frame_id))
            points = np.fromfile(bin_path, dtype=np.float32).reshape(-1, 4)
        else:
            bin_path = osp.join(self.data_ceph, seq_id, 'lidar_roof', '{}.bin'.format(frame_id))
            points = np.copy(self.petrel_client.load_bin(bin_path, dtype='float32').reshape(-1, 4))
        return points
